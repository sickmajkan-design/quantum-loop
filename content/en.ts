import type { Dictionary } from "./types";

const en: Dictionary = {
  nav_services: "Services",
  nav_work: "Work",
  nav_about: "About",
  nav_contact: "Contact",
  nav_cta: "Get a quote",

  badge: "20+ years of experience",
  hero_title: "Design that |sticks.",
  hero_sub:
    "Graphic design, signage of all kinds, vehicle wrapping and window tinting. From idea to installation — with a warranty on every job.",
  hero_cta1: "Get a quote",
  hero_cta2: "See services",

  s_eyebrow: "Services",
  s_title: "If it's visible — we make it.",
  s1_t: "Graphic design",
  s1_p: "Logos, visual identities and print-ready artwork. Design that makes your brand instantly recognizable.",
  s2_t: "Vehicle wrapping",
  s2_p: "Full and partial vinyl wraps — cars, vans, trucks. Your vehicle becomes an ad that works 24/7.",
  s3_t: "Vinyl on any surface",
  s3_p: "Shop windows, glass, walls and facades. Precise, bubble-free installation on any flat surface.",
  s3_shop_label: "Shop windows & glass",
  s3_shop_alt: "Hands applying vinyl film to a glass shop window surface",
  s3_sun_label: "Sun protection — facades",
  s3_ppf_label: "PPF paint protection",
  s4_t: "Window tinting",
  s4_p: "Professional tinting for vehicles and buildings. Sun protection, privacy and a modern look — certified films.",
  s4_before: "Clear",
  s4_after: "Tinted",
  s5_t: "Signage of all kinds",
  s5_p: "Illuminated signs, 3D letters, banners, boards and interior branding. From production to installation.",

  g_eyebrow: "Why us",
  g_title: "years of craft. Warranty on every job.",
  g1_t: "Warranty",
  g1_p: "Written warranty on every completed job — materials and installation.",
  g2_t: "Experience",
  g2_p: "Over 20 years in the trade. We know what works.",
  g3_t: "Full service",
  g3_p: "Design, production and installation — all in one place.",

  p_eyebrow: "Work",
  p_title: "Our work speaks for itself.",
  p_insta_title: "Follow us on Instagram",
  p_insta_text: "Videos, reels and stories from every job — see the process live.",
  p_insta_cta: "Open Instagram",
  p_soon: "Photos coming soon",
  p_soon_short: "Soon",

  sz_eyebrow: "New",
  sz_title: "Coming soon to Salzburg.",
  sz_p: "Quality proven in Bosnia is coming to Austria. Quantum Loop will soon serve clients in Salzburg and the surrounding area.",

  faq_eyebrow: "Questions",
  faq_title: "Frequently asked questions.",
  faq1_q: "How long does production and installation take?",
  faq1_a:
    "It depends on the scope — simple stickers and smaller signage are ready in a few days, while a full vehicle wrap or larger facade vinyl can take one to two weeks. You'll get an exact timeline with your quote.",
  faq2_q: "Do you guarantee the finished work?",
  faq2_a: "Yes — a written warranty on materials and installation, on every job, no exceptions.",
  faq3_q: "Do you come on-site for measuring and installation?",
  faq3_a:
    "Yes, we come to you in Derventa and the surrounding area (soon in Salzburg too) — measuring, installation, and everything else happens on location.",
  faq4_q: "Which areas do you cover?",
  faq4_a:
    "We're based in Derventa, Bosnia and Herzegovina, work across all of BiH, and will soon also serve clients in Salzburg and the surrounding area, Austria.",
  faq5_q: "Do you take on smaller jobs, or only big projects?",
  faq5_a:
    "We do it all, from a single sticker or sign to a complete vehicle or facade wrap — no job is too small.",

  c_eyebrow: "Contact",
  c_title: "Request a free quote.",
  c_soon: "Soon: Salzburg, Austria",

  f_name: "Full name",
  f_email: "Email",
  f_phone: "Phone (optional)",
  f_service: "Service",
  f_dimensions: "Dimensions (approx.)",
  f_quantity: "Quantity",
  f_deadline: "Deadline",
  f_dl_flexible: "Flexible",
  f_dl_2w: "Within 2 weeks",
  f_dl_1w: "Within 1 week",
  f_dl_asap: "Urgent",
  f_install: "Installation",
  f_install_yes: "Yes, needed",
  f_install_no: "No, production only",
  f_install_unsure: "Not sure",
  f_message: "Message",
  f_msg: "Describe what you need...",
  f_send: "Send inquiry",
  f_sending: "Sending…",
  form_ok: "Thank you! We've received your request and will get back to you as soon as possible.",
  form_err: "Something went wrong — please try again later.",
  skip: "Skip to content",
  f_tag: "Design · Signage · Vinyl · Tinting",

  svc: [
    "Graphic design",
    "Vehicle wrapping",
    "Vinyl on surfaces",
    "Window tinting",
    "Signage",
  ],
  tiles: [
    "Vehicle wrap",
    "Illuminated sign",
    "Shop window vinyl",
    "Tinting",
    "Logo design",
    "3D letters",
  ],
};

export default en;
