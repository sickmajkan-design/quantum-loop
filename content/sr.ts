import type { Dictionary } from "./types";

const sr: Dictionary = {
  nav_services: "Usluge",
  nav_work: "Radovi",
  nav_about: "O nama",
  nav_contact: "Kontakt",
  nav_cta: "Zatraži ponudu",

  badge: "20+ godina iskustva",
  hero_title: "Dizajn koji se |lijepi.",
  hero_sub:
    "Grafički dizajn, reklame svih vrsta, brendiranje vozila i zatamnjivanje stakala. Od ideje do montaže — sa garancijom na svaki urađeni posao.",
  hero_cta1: "Zatraži ponudu",
  hero_cta2: "Pogledaj usluge",

  s_eyebrow: "Usluge",
  s_title: "Sve što se vidi — mi radimo.",
  s1_t: "Grafički dizajn",
  s1_p: "Logotipi, vizuelni identiteti i pripreme za štampu. Dizajn koji vaš brend čini prepoznatljivim na prvi pogled.",
  s2_t: "Brendiranje vozila",
  s2_p: "Kompletno i djelimično oblaganje folijom — automobili, kombiji, kamioni. Vaše vozilo postaje reklama koja radi 24/7.",
  s3_t: "Folije na sve površine",
  s3_p: "Izlozi, staklene površine, zidovi i fasade. Precizna montaža bez mjehurića — na bilo kojoj ravnoj površini.",
  s4_t: "Zatamnjivanje stakala",
  s4_p: "Profesionalno zatamnjivanje stakala vozila i objekata. Zaštita od sunca, privatnost i moderan izgled — atestirane folije.",
  s5_t: "Reklame svih vrsta",
  s5_p: "Svijetleće reklame, 3D slova, baneri, table i unutrašnje brendiranje kompanija. Od proizvodnje do montaže.",

  g_eyebrow: "Zašto mi",
  g_title: "godina zanata. Garancija na svaki posao.",
  g1_t: "Garancija",
  g1_p: "Pisana garancija na svaki urađeni posao — materijal i montažu.",
  g2_t: "Iskustvo",
  g2_p: "Više od 20 godina u zanatu. Znamo šta radi, a šta ne.",
  g3_t: "Kompletna usluga",
  g3_p: "Dizajn, proizvodnja i montaža — sve na jednom mjestu.",

  p_eyebrow: "Radovi",
  p_title: "Naš rad govori umjesto nas.",
  p_insta_title: "Prati nas na Instagramu",
  p_insta_text:
    "Video, reels i storyji sa svakog projekta — pogledaj kako radimo, uživo.",
  p_insta_cta: "Otvori Instagram",

  sz_eyebrow: "Novo",
  sz_title: "Uskoro u Salzburgu.",
  sz_p: "Kvalitet dokazan u BiH stiže u Austriju. Quantum Loop uskoro otvara vrata i klijentima u Salzburgu i okolini.",

  c_eyebrow: "Kontakt",
  c_title: "Zatražite besplatnu ponudu.",
  c_soon: "Uskoro: Salzburg, Austrija",

  f_name: "Ime i prezime",
  f_email: "Email",
  f_msg: "Opišite šta vam treba...",
  f_send: "Pošalji upit",
  form_ok: "Hvala! Javićemo vam se uskoro.",
  f_tag: "Dizajn · Reklame · Folije · Zatamnjivanje",

  svc: [
    "Grafički dizajn",
    "Brendiranje vozila",
    "Folije na površine",
    "Zatamnjivanje stakala",
    "Reklame",
  ],
  tiles: [
    "Brendiranje vozila",
    "Svijetleća reklama",
    "Izlog — folija",
    "Zatamnjivanje",
    "Logo dizajn",
    "3D slova",
  ],
};

export default sr;
